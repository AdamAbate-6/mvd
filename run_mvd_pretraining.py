...
def get_args():
    ...
    parser.add_argument(
        "--image_teacher_model",
        default="mae_teacher_vit_base_patch16",
        type=str,
        metavar="MODEL",
...
def main(args):
    ...
    if args.image_teacher_model_ckpt_path:
        if args.image_teacher_model_ckpt_path.startswith("https"):
            ...
            checkpoint = torch.load(
                args.image_teacher_model_ckpt_path, map_location="cpu"
            )
        ...
        utils.load_state_dict(
            image_teacher_model, checkpoint_model, prefix=args.model_prefix
        )
    ...
    if args.video_teacher_model_ckpt_path:
        if args.video_teacher_model_ckpt_path.startswith("https"):
            ...
            checkpoint = torch.load(
                args.video_teacher_model_ckpt_path, map_location="cpu"
            )
        ...
        utils.load_state_dict(
            video_teacher_model, checkpoint_model, prefix=args.model_prefix
        )
    ...
    total_batch_size = (
        args.batch_size * args.num_sample * args.update_freq * utils.get_world_size()
    )
    num_training_steps_per_epoch = len(dataset_train) // int(
        total_batch_size / args.num_sample
    )
    ...
    if args.output_dir and utils.is_main_process():
        with open(
            os.path.join(args.output_dir, "config.txt"), mode="a", encoding="utf-8"
        ) as f:
