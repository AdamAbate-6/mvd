    ...
    def __init__(
        ...
        if self.data_path is not None:
            self.dataset_samples = [
                os.path.join(self.data_path, p) for p in self.dataset_samples
            ]
    ...
    def __getitem__(self, index):
        ...
        if self.num_sample > 1:
            ...
            for idx in range(self.num_sample):
                process_data_0, process_data_1, mask = self.transform(
                    (images[idx * self.new_length : (idx + 1) * self.new_length], None)
                )  # T*C,H,W
                process_data_0 = process_data_0.view(
                    (self.new_length, 3) + process_data_0.size()[-2:]
                ).transpose(
                    0, 1
                )  # T*C,H,W -> T,C,H,W -> C,T,H,W
                process_data_1 = process_data_1.view(
                    (self.new_length, 3) + process_data_1.size()[-2:]
                ).transpose(
                    0, 1
                )  # T*C,H,W -> T,C,H,W -> C,T,H,W
